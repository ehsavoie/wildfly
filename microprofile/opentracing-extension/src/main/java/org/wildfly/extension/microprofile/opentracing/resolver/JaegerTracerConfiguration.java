...
public class JaegerTracerConfiguration implements TracerConfiguration {
    ...
    public JaegerTracerConfiguration(OperationContext context, ModelNode configuration) throws OperationFailedException {
        ...
        tracerTags = new HashMap<>();
        ModelNode tags = TRACER_TAGS.resolveModelAttribute(context, configuration);
        if (tags.isDefined()) {
            for (Property prop : tags.asPropertyList()) {
                tracerTags.put(prop.getName(), prop.getValue().asString());
            }
        }
